...
function _ifIn(key, list, options) {
  for(var index in list) {
    if(list[index][key]){
      return options.fn(list[index][key]);
    }
  }
  return options.inverse(this);
}
...
function getDefaultConfig(https, mainTemplate, resourceTemplate, itemTemplate, responseTemplate) {
    ...
    if (typeof responseTemplate !== 'function') {
        responseTemplate = fs.readFileSync(responseTemplate || __dirname + '/response.handlebars', 'utf8');
    }
